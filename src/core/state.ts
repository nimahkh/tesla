...
export class State_NoProxy<T extends object> {
  ...
  private reactiveExpressions: Record<
    string,
    { expression: string; dependencies: string[] }
  > = {};
  ...
  private evaluateExpression(key: string) {
    ...
    const evalResult = new Function("state", `return ${expression};`)(
      this.state,
    );
  ...
  private triggerListeners(key: string) {
    if (this.listeners[key]) {
      this.listeners[key].forEach((callback) =>
        callback(this.getValue(this.state, key.split("."))),
      );
    }
  }
