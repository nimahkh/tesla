...
describe('StateManager without IE11 considerations', () => {
  ...
  beforeEach(() => {
    stateManager = new StateManager<{ count: number, nested: { value: boolean } }>({
      count: 0,
      nested: { value: true },
    });
  });
  ...
  test('initial state is set correctly', () => {
    expect(stateManager.getState()).toEqual({ count: 0, nested: { value: true } });
  });
  ...
  test('subscribers are notified on state change', () => {
    ...
    expect(subscriberMock).toHaveBeenCalledWith({ count: 5, nested: { value: true } });
  });
});
